...
class MoveParticle extends Operation {
	...
	public MoveParticle(FramesData framesData, int currentFrame, Particle source) {
		this.framesData = framesData;
		this.currentFrame = currentFrame;
		this.source = source;
		oldPosition = new Point(source.position);
		newPosition = new Point(source.position);
	}
	...
	public void mouseDragEnd(BetterMouseEvent mouseEvent) {
		...
		if ((clickedParticle = framesData.frames[currentFrame].getParticleInPosition(mouseEvent.position)) != null) {
			cancel();
	...
	static void startOperation(OperationManager operationManager, BetterMouseEvent mouseEvent, FramesData frames,
		...
		if (mouseEvent.leftButton) {
			...
			if ((clickedParticle = frames.frames[currentFrame].getParticleInPosition(mouseEvent.position)) != null) {
				operationManager.startOperation(new MoveParticle(frames, currentFrame, clickedParticle));
			}
		}
	}
...
class SwapIdentities extends InstantOperation {
	...
	@Override
	void undo() {
		for (Particle a : affectedA) {
			a.identity = identityA;
		}
		for (Particle b : affectedB) {
...
class BringParticle extends Operation {
	...
	BringParticle(FramesData framesData, int currentFrame, Frame frame, Particle particle) {
		this.framesData = framesData;
		this.currentFrame = currentFrame;
		this.frame = frame;
		this.particle = particle;
		newPosition = new Point(particle.position);
	}
	...
	static void startOperation(OperationManager operationManager, BetterMouseEvent mouseEvent, FramesData frames,
		...
		if (mouseEvent.leftButton) {
			...
			if (currentFrame > 0
				...
				operationManager.startOperation(
						new BringParticle(frames, currentFrame, frames.frames[currentFrame], clickedParticle));
			}
		}
	}
